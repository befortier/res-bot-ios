...
public struct BulkNotificationFlowViewModel {
    ...
    public func submit(from state: BulkNotificationFlowView.ViewState) async throws -> Int {
        try await submitter(
        ...
        return state.selectedVenueIDs.count * abs(state.partySizeRange.upperBound.distance(to: state.partySizeRange.lowerBound))
    }
}
