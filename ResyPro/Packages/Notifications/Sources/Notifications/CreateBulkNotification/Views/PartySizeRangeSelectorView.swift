...
public struct PartySizeRangeSelectorView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 16) {
                Stepper("Min: \(partySizeRange.lowerBound)", value: lowerBinding, in: min...partySizeRange.upperBound)
                Stepper("Max: \(partySizeRange.upperBound)", value: upperBinding, in: partySizeRange.lowerBound...max)
            }
        }
    }
...
fileprivate struct PartySizeRangeSelectorPreview: View {
    @State private var partySizeRange = ClosedRange<Int>(uncheckedBounds: (lower: 1, upper: 10))
    var body: some View {
        PartySizeRangeSelectorView(partySizeRange: $partySizeRange, rangeBounds: .init(uncheckedBounds: (lower: 1, upper: 10)))
    }
}
